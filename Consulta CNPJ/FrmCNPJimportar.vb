...
Public Class FrmCNPJimportar
    ...
    Private Sub BtnExterno_Click(sender As Object, e As EventArgs) Handles BtnExterno.Click
        ...
        ElseIf Application.OpenForms.OfType(Of FrmParcelamento)().Any() Then
            ...
            Me.Close()
        Else
            ' Se nenhum formulário estiver aberto, avisa o usuário
            MessageBox.Show("Nenhum formulário de Legalização ou Parcelamento está aberto. Por favor, abra um formulário para continuar.", "Formulário Não Encontrado", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If
    End Sub
    ...
    Private Sub BtnInterno_Click(sender As Object, e As EventArgs) Handles BtnInterno.Click
        ...
        If Application.OpenForms.OfType(Of FrmLegalizacao)().Any() Then
            ...
            End If
        ElseIf Application.OpenForms.OfType(Of FrmParcelamento)().Any() Then
            Dim CNPJ As String = FrmParcelamento.CNPJMaskedTextBox.Text.Replace("/", "").Replace(",", "").Replace("-", "").Replace(".", "")
    ...
    Private Sub BtnImportarFederal_Click(sender As Object, e As EventArgs) Handles BtnImportarFederal.Click
        ...
        ' Verificar se o FrmParcelamento está aberto
        If Application.OpenForms.OfType(Of FrmParcelamento)().Any() Then
            MessageBox.Show("Em breve disponível para uso.", "Em Breve", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If
    ...
    Private Sub ButtonCadEmpresa_Click(sender As Object, e As EventArgs) Handles ButtonCadEmpresa.Click

        Dim cnpj As String = String.Empty

        ' Verificar qual formulário está aberto para obter o CNPJ
        If Application.OpenForms.OfType(Of FrmLegalizacao)().Count() > 0 Then
            cnpj = FrmLegalizacao.CNPJMaskedTextBox.Text
        ElseIf Application.OpenForms.OfType(Of FrmParcelamento)().Count() > 0 Then
            cnpj = FrmParcelamento.CNPJMaskedTextBox.Text
        ElseIf Application.OpenForms.OfType(Of FrmAlvara)().Count() > 0 Then
            cnpj = FrmAlvara.CNPJMaskedTextBox.Text
        End If

        ' Se não encontrar nenhum formulário aberto, pegar o CNPJ do CNPJMaskedTextBox atual
        If String.IsNullOrEmpty(cnpj) Then
            MessageBox.Show("Nenhum formulario aberto!", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If

        ' Validar se o CNPJ foi informado
        If String.IsNullOrEmpty(cnpj) Then
            MessageBox.Show("Por favor, insira ou selecione um CNPJ válido.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If


        ' Chamar o método VerificarCNPJ
        If VerificarCNPJ(cnpj) Then
            MessageBox.Show("CNPJ já cadastrado em Empresas.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information)
            If Application.OpenForms.OfType(Of FrmLegalizacao)().Count() > 0 Then
                FrmLegalizacao.Focus()
            Else
                ' Dim frm As New FrmLegalizacao()
                FrmLegalizacao.CNPJMaskedTextBox.Text = cnpj
                FrmLegalizacao.Show()
            End If
        Else
            ' Abrir o formulário FrmLegalizacao se o CNPJ não estiver cadastrado
            If Application.OpenForms.OfType(Of FrmLegalizacao)().Count() > 0 Then
                FrmLegalizacao.Focus()
                FrmLegalizacao.BtnNovo.PerformClick()
            Else
    ...
    Private Function VerificarCNPJ(cnpj As String) As Boolean
        ' O CNPJ está no formato já formatado com máscara
        Dim cnpjFormatado As String = cnpj.Trim()

        ' String de conexão ao banco de dados (substitua 'str' pela sua connection string)
        Dim str As String = "SuaConnectionStringAqui"
        ...
        Using connection As New SqlConnection(str)
            Try
                ...
                Dim query As String = "SELECT RazaoSocial FROM Empresas WHERE CNPJ = @CNPJ"
                Using cmd As New SqlCommand(query, connection)
                    cmd.Parameters.AddWithValue("@CNPJ", cnpjFormatado)
                    ...
                    If razaoSocial IsNot Nothing Then
                        ' Log do resultado encontrado
                        Debug.WriteLine("CNPJ encontrado: " & cnpjFormatado)

                        ' Exibir mensagem com a razão social
                        MessageBox.Show("Empresa já cadastrada!" & vbCrLf &
                                        "CNPJ: " & cnpjFormatado & vbCrLf &
                                        "Razão Social: " & Convert.ToString(razaoSocial),
                                        "Informação",
                                        MessageBoxButtons.OK,
                                        MessageBoxIcon.Information)
                        Return True
                    Else
                        ' Exibir mensagem indicando que o CNPJ não foi encontrado
